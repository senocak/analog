---
name: spring-boot
description: "Spring Boot 3.5.x development - REST APIs, JPA, Security, Testing, and Cloud-native patterns. Use for building enterprise Java applications with Spring Boot."
---

# Spring Boot Skill

Enterprise Spring Boot 3.5.x development with focus on clean architecture and production-ready code.
...
public class ProductService {
    ...
    public List<Product> search(final String name) {
...
public class ProductController {
    ...
    @GetMapping
    public List<Product> search(final @RequestParam(defaultValue = "") String name) {
        return service.search(name);
    }

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public Product create(final @Valid @RequestBody ProductRequest request) {
...
public class GlobalExceptionHandler {
    ...
    public Map<String, String> handleNotFound(final EntityNotFoundException ex) {
...
public class OrderController {
    ...
    @GetMapping("/{id}")
    public Mono<ResponseEntity<OrderDto>> getOrder(final @PathVariable UUID id) {
    ...
    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    public Mono<OrderDto> createOrder(final @Valid @RequestBody CreateOrderRequest request) {
...
Spring Boot 3.5.x, Java 25, Spring WebFlux, Project Reactor, Spring Data JPA, Spring Security 6, OAuth2/JWT, Hibernate, R2DBC, Spring Cloud, Resilience4j, Micrometer, JUnit 5, TestContainers, Mockito, Maven/Gradle