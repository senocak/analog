...
class FileStorageService {
    fun saveCover(file: MultipartFile?, postId: String): String {
        ...
        val target = Path.of("data/uploads/covers/$postId.jpg")
        Files.createDirectories(target.parent)
        writeJpeg(file, target, 1024)
    ...
    fun savePhoto(file: MultipartFile): String {
        ...
        val target = Path.of("data/uploads/images/${now.year}/${"%02d".format(now.monthValue)}/$filename")
        Files.createDirectories(target.parent)
        writeJpeg(file, target, 2000)
    ...
    fun resourceUnder(base: Path, relativePath: String): Resource {
        ...
        val target = normalizedBase.resolve(relativePath.removePrefix("/")).normalize()
    ...
    private fun writeJpeg(file: MultipartFile, target: Path, maxWidth: Int) {
        ...
        val width = minOf(original.width, maxWidth)
        val height = (original.height.toDouble() * width.toDouble() / original.width.toDouble()).toInt().coerceAtLeast(1)
